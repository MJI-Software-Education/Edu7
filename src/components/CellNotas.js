...
export const CellNotas = ({n, a, prueba, curso, asignatura}) => {

    const dispatch = useDispatch();
    const [ form, onChange ] = useForm({
        valor: ( a._id == n.idUsuario) ? n.nota : 1
    });
    ...
    return (
        <div>
            <TableCell key={n.idUsuario}> <input className="rounded" type="text" name='valor' value={form.valor} onChange={onChange} onKeyDown={(e) => handleChangeNota(e,a._id)} min="1.0" step="0.1" max="7.0" id={a._id} placeholder="6.5" /> </TableCell>
        </div>
    )
}
